...
def run_pytest(target: Path, label: str) -> tuple[int, str]:
    """Run pytest on ``target`` (file or dir). Returns (exit_code, last_line_or_summary)."""
    cmd = [
        sys.executable,
        "-m",
        "pytest",
    ...
    return proc.returncode, tail or f"exit {proc.returncode}"
...
def main() -> int:
    ...
    if not UNIT_DIR.is_dir():
        ...
        code, tail = run_pytest(UNIT_DIR, "unit")
    ...
    if not INTEGRATION_DIR.is_dir():
        ...
        code, tail = run_pytest(INTEGRATION_DIR, "integration")
        print(" ", parse_pytest_summary_line(tail))
        if code != 0:
            overall_fail += 1
            print(f"  [FAIL] pytest exit code {code}")
