...
if __name__ == "__main__":
    ...
    id = 0
    for _ in range(30):
        actions: List[AddResLayer] = AddResLayer.generate_all_actions(gm, layer_types=[Layer_Type.RANDOM])
        id += 1
        idx = rng.randrange(len(actions))
        print(f"idx: {id} " + "--------------------------------")
        print(f"gm.graph: {gm.graph}")
        print("action used: ", actions[idx])
        draw_filtered_fx_graph(gm, "fx_graph_simplified" + str(id), fmt="pdf")
        draw_torch_fx_graph(gm, "fx_graph" + str(id), fmt="pdf")
