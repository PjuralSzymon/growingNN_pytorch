...
`SimulationScore` in `growingnn/simulation/score_functions/simulation_score.py` ranks a candidate model after a simulated mutation. `montecarlo_alg.py` and `greedy_alg.py` call `simulation_score.score(model, ctx)` on each rollout.
...
Sub-scores live in [[Score by learning]] and [[Score by effitiency]]. All of them read `RunningConfig` for epochs, LR, `criterion`, and simulation loaders set via `set_simulation_loaders` in `train_generations`.
