...
Both call `gradient_descent` for `simulation_scheduler.simulation_epochs` on `RunningConfig.sim_train_loader` and `sim_val_loader` (sampled [[Simulation Set]] in `train_generations`). LR comes from `RunningConfig.lr_scheduler`.

- `score_acc` returns the last `val_acc` from the history (higher is better).
- `score_loss` returns `min(1 / max(val_loss, 1e-8), 1.0)` so loss is mapped to a bounded reward.
