...
Both call `gradient_descent` for `simulation_scheduler.simulation_epochs` on `RunningConfig.sim_train_loader` and `sim_val_loader` (set via `set_simulation_loaders` in `train_generations`). LR comes from `RunningConfig.lr_scheduler`.
