...
Used by [[Sequential Conv Action]], [[Residual Linear Actions]], [[Residual Conv Action]], and [[Del Layer Action]].
...
`find_seq_linear_after_conv_sizes` does not add pool or flatten. It sizes a plain `nn.Linear` on the existing FX path (see [[Sequential Linear Actions]]).
