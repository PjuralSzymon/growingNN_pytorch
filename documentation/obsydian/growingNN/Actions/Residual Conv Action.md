...
It depends on [[Model Analyser]] for `module_dependency_pairs` and `get_layer_module`. It depends on [[Layer Analyser]] for `get_layer_output_shapes`. Execution calls `add_new_residual_layer` in [[Model Transformer]]. Factories live in [[Layer Factory]] (`ConvFactory.create_zero_conv`, `ConvFactory.create_zero_conv_before_linear`). Names come from [[Name factory]]. Width divisibility uses [[Conv to linear adapter]].
...
It reads pairs from `module_dependency_pairs(gm)`. It reads shapes from `get_layer_output_shapes(gm)`. For each `(layer_from_id, layer_to_id)` it loads modules with `get_layer_module(layer_from_id, model)` and `get_layer_module(layer_to_id, model)`.
...
If `layer_to` is conv and `out_shapes` is non-empty (lines 50 to 54), a pair is kept only when both layers have a recorded shape and those tuples are equal (`s_from == s_to`). Missing shapes or unequal shapes skip the pair. That removes cross-stage ResNet pairs that would break `torch.add`. See [[Layer Analyser]] section on the conv residual check.
...
1. Conv to linear path has no `get_layer_output_shapes` equality check in `add_res_conv_layer.py` lines 64 to 76; execute-time errors are still possible there.

2. When `get_layer_output_shapes` returns `{}`, conv-conv filtering is disabled; rare models may still throw on add.
