...
Pass an `ActionLearningRateScheduler` (subtype of `LearningRateScheduler`) on `RunningConfig.lr_scheduler`. Modes live in `ScheduleMode` in `lr_scheduler_action.py`:
...
`train_generations` calls `structure_changed()` only after an architecture action runs. That resets warmup. See [[Training loop]].
...
config.lr_scheduler = ActionLearningRateScheduler(
...
`ReduceLROnPlateau` and `OneCycleLR` are not wrapped yet. They need different trainer hooks than epoch-index global adapters.
