...
class El2nSimulationSet(SimulationSet):
    """
    7. EL2N.

    Paper: Paul et al. 2021, NeurIPS 34, 20596-20607 (paul2021dataDiet).

    Keep class-balanced examples whose softmax is farthest from the one-hot label.

    Pseudocode:
        for each sample:
            EL2N = || softmax(model(x)) - one_hot(y) ||_2
        ...
        try:
            with torch.no_grad():
                for batch in scoring_loader(train_loader.dataset, train_loader.batch_size or 32):
                    ...
                    targets = F.one_hot(y, num_classes=probs.shape[1]).float()
                    scores = torch.norm(probs - targets, p=2, dim=1)
                    for label, score in zip(y.tolist(), scores.detach().cpu().tolist()):
